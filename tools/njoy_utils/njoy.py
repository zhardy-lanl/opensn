...
class NJOY:
    ...
    def _reconr(self, rxn):
        """Returns a RECONDR input for the given reaction type.

        :param rxn: The reaction type for this RECONR input.
        :type rxn:  str, {'neutron', 'gamma', 'photoat'}
        :return: The RECONR input lines.
        :rtype: list of str
        """
        data = self._get_data(rxn)

        lines = ['reconr']
        lines += ['-21 -22/' if rxn == 'neutron' else
                  '-61 -62/' if rxn == 'gamma' else
                  '-71 -72/']
        lines += [f'\'pendf {rxn} tape\'']
        lines += [f'{data.matnum} 0/']
        lines += ['0.001/']
        lines += ['0/']
        return lines

    def _broadr(self, rxn):
        """Returns a BROADR input for the given reaction type.

        :param rxn: The reaction type for this BROADR input.
        :type rxn:  str, {'neutron', 'gamma'}
        :return: The BROADR input lines.
        :rtype: list of str
        """
        data = self._get_data(rxn, ['neutron', 'gamma'])

        lines = ['broadr']
        lines += ['-21 -22 -23/' if rxn == 'neutron' else '-61 -62 -63/']
    ...
    def _groupr(self, rxn):
        """Returns a GROUP input for the given reaction type.

        :param rxn: The reaction type for this GROUP input.
        :type rxn:  str, {'neutron', 'gamma'}
        ...
        lines += [f'{data.matnum} {self._neutron.grps_opt} {self._gamma.grps_opt} '
        ...
        if rxn == 'neutron' and self._with_thermal:
            lines += ['3 221 \'free gas\'/']
            if self._thermal:
                lines += [f'3 {self._thermal.mti} \'inelastic s(a,b)\'/']
                if self._thermal.mtc:
                    lines += [f'3 {self._thermal.mtc} \'elastic s(a,b)\'/']

        # Fission cross-sections
        if self._is_fissionable:
            lines += [f'3 452 \'total nubar ({rxn})\'/']
            lines += [f'3 456 \'prompt nubar ({rxn})\'/']
            lines += [f'5 18 \'prompt chi ({rxn})\'/']
            lines += [f'3 455 \'delayed nubar ({rxn})\'/']
            lines += [f'5 455 \'delayed chi ({rxn})\'/']

        # Energy/speed data
        lines += [f'3 257 \'average energy ({rxn})\'/']
        if rxn == 'neutron':
            lines += ['3 259 \'inverse velocity (neutron)\'/']
        ...
        if self._is_fissionable:
            lines += [f'6 18 \'({rxn[0]},fission) neutron matrix\'/']

        # Neutron-to-gamma transfer matrices
        if rxn == 'neutron' and self._gamma:
        ...
        lines += ['moder']
        lines += ['-28 90/' if rxn == 'neutron' else '-64 91/']
        return lines
    ...
    def _macxsr(self, rxn):
        """Returns the MATXSR input for the given reaction type.

        :param rxn: The reaction type for this MACXSR input.
        :type rxn:  str, {'neutron'}
        :return: The MACXSR input lines.
        :rtype: list of str
        """
        data = self._get_data(rxn, ['neutron'])
        lines = ['matxsr']
        lines += ['90 0 97/']
        lines += ['0 \'LANL NJOY\'/']
        ...
        lines += ['1/' if not self._with_thermal else '1 1/']
        lines += [f'{self._symbol.upper()}{self._mass_number} {self._neutron.matnum}/']
        return lines

    def _get_data(self, rxn, allow_list=None):
        """
        :param rxn: A reaction type
        :type rxn:  str, {'neutron', 'gamma', 'photoat'}
        ...
        if rxn not in allow_list:
            raise ValueError(f'Reaction type must be one of {allow_list}.')

        if rxn == 'neutron' and self._neutron:
            return self._neutron
        elif rxn == 'gamma' and self._gamma:
            return self._gamma
        elif rxn == 'photoat' and self._photoat:
            return self._photoat
        else:
            raise AssertionError(f'The requested data for {rxn} is not available.')
