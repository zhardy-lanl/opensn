  ...
  std::optional<const std::reference_wrapper<std::vector<double>>> opt_src)
{
  ...
  for (const auto& cell : grid.local_cells)
  {
    ...
    for (uint64_t i = 0; i < discretization.GetCellNumNodes(cell); ++i)
      for (uint64_t m = 0; m < num_moments; ++m)
        ...
        {
          cell_ids.push_back(cell_global_id);
          nodes.push_back(i);
          moments.push_back(m);
          groups.push_back(g);
  ...
  H5WriteDataset1D(file_id, "cell_ids", cell_ids);
  H5WriteDataset1D(file_id, "nodes", nodes);
  H5WriteDataset1D(file_id, "moments", moments);
                             ...
                             std::optional<std::reference_wrapper<std::vector<double>>> opt_dest)
{
  ...
  const auto cell_ids = H5ReadDataset1D<uint64_t>(file_id, "cell_ids");
  const auto nodes = H5ReadDataset1D<uint64_t>(file_id, "nodes");
  const auto moments = H5ReadDataset1D<uint64_t>(file_id, "moments");
  const auto groups = H5ReadDataset1D<uint64_t>(file_id, "groups");
  ...
  for (uint64_t i = 0; i < num_local_dofs; ++i)
  {
    // Only set local values
    if (grid.IsCellLocal(cell_ids[i]))
    {
      const auto& cell = grid.cells[cell_ids[i]];
      const auto dof_map =
        discretization.MapDOFLocal(cell, nodes[i], uk_man, moments[i], groups[i]);
      dest[dof_map] = values[i];
    }
    H5Fclose(file_id);
  }
}
